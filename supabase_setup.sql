...
-- 2.1. Create a table for user permissions in forms
...
comment on table user_form_permissions is 'Manages which user has access to which form.';
...
alter table user_form_permissions enable row level security;
...
-- Create policies for 'user_form_permissions' table
-- Allow users to read their own form permissions.
create policy "Allow users to read their own form permissions" on user_form_permissions
  for select using (auth.uid() = user_id);
  
-- Allow service_role (admins from the server) to manage all form permissions.
create policy "Allow admin full access to form permissions" on user_form_permissions
  for all using (auth.role() = 'service_role');
